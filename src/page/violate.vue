                ...
                <el-table-column type="expand">
                  <template slot-scope="scope" v-if="scope.row.noticeType == 1 && scope.row.noticeStatus != 2">
                    <el-form label-position="left" inline class="demo-table-expand">
                      ...
                      <img v-if="adDetail.adPicture" :src="adDetail.adPicture" style="width: 200px;height: 450px;">
                ...
                <el-table-column label="操作" width="270" align="center">
                  <template slot-scope="scope">
                    <el-button
                      size="mini"
                      type="info"
                      v-if="scope.row.noticeStatus == 0 && (user.userType == 0 || user.userType == 3)"
                      @click="handleEdit(scope.$index, scope.row, 'reject')">驳回</el-button>
                    <el-button
                      size="mini"
                      type="warning"
                      v-if="scope.row.noticeStatus == 0 && (user.userType == 0 || user.userType == 3)"
                      @click="handleEdit(scope.$index, scope.row, 'delete')">删除广告</el-button>
                    <el-button
                      size="mini"
                      type="danger"
                      v-if="scope.row.noticeStatus == 0 && (user.userType == 0 || user.userType == 3)"
                      @click="handleEdit(scope.$index, scope.row, 'forbid')">店铺封禁3天</el-button>
                    <el-button
                      size="mini"
                      type="text"
                    ...
                    <el-button
                      ...
                      @click="">已删除广告</el-button>
                    <el-button
                      size="mini"
                      type="text"
                      v-if="scope.row.noticeStatus == 3"
                      @click="">已将店铺封禁3天</el-button>
                  </template>
                </el-table-column>
            </el-table>
...
<script>
    ...
    import {cityGuess, getResturants, getResturantsCount, foodCategory, updateResturant, searchplace, deleteResturant, shopList, timestampToTime, getUserTypeName, violateList, getAdByAdId, getShopOrAdTypeName, reject, deleteAd, forbidShop} from '@/api/getData'
    export default {
        data(){
            return {
                baseUrl,
        ...
        methods: {
            ...
            async getViolateList(){
                ...
                const res = await violateList({userId: this.user.userId, current: this.currentPage, size: this.limit});
                let index = 0
                this.tableData = [];
            ...
            async handleEdit(index, row, operation) {
              ...
              if (operation == 'reject') {
                console.log(1)
                const res = await reject(operationDTO)
                code = res.data.code
              }
              if (operation == 'delete') {
                console.log(2)
                operationDTO.target = row.adId
                const res = await deleteAd(operationDTO)
                code = res.data.code
              }
              if (operation == 'forbid') {
                console.log(3)
                operationDTO.target = row.shopId
                const res = await forbidShop(operationDTO)
                code = res.data.code
              }

              let msg = ''
              switch(code) {
                case 61: msg = "已驳回"; break;
                case 61: msg = "驳回失败"; break;
                case 33: msg = "已删除广告"; break;
                case 32: msg = "删除广告失败"; break;
                case 23: msg = "停封3天"; break;
                case 22: msg = "停封失败"; break;
              }
              this.getViolateList()
              this.$message({
                type: 'info',
